...
for page_number in page:
    ...
    url = "https://search.shopping.naver.com/search/all?exrental=true&exused=true&frm=NVSHATC&origQuery=%EB%85%B8%ED%8A%B8%EB%B6%81&pagingIndex=" + str(
        page_number) + "&pagingSize=40&productSet=total&query=%EB%85%B8%ED%8A%B8%EB%B6%81&sort=rel&timestamp=&viewType=list"
    html = urlopen(url)
...
df_notebook = pd.DataFrame(file_data,
                           columns=["PRODUCT_NAME", "PRICE", "DISPLAY", "CPU_NAME", "RESOLUTION", "RAM", "STORAGE_SIZE",
                                    "WEIGHT"])
